...
async def get_debts_with_person(kakao_id: str, person_kakao_id: str):
    logger.debug(f"{kakao_id}와 {person_kakao_id} 사이의 채무를 가져오는 중")
    ...
    logger.debug(f"사용자 닉네임: {user_nickname}, 상대방 닉네임: {person_nickname}")
    ...
    # 각 채무의 from_user와 to_user 로그 기록
    for debt in debts_with_person:
        logger.debug(f"채무 ID: {debt['_id']}, from_user: {debt['from_user']}, to_user: {debt['to_user']}, amount: {debt['amount']}")
        debt["_id"] = str(debt["_id"])

    logger.debug(f"조회된 채무: {debts_with_person}")
...
async def get_user_category_expenses(kakao_id: str):
    ...
    # 카테고리별 데이터 집계
    category_data = {category: 0 for category in fixed_colors.keys()}  # 미리 정의된 카테고리로 초기화
        ...
        for participant in expense["participants"]:
            if participant["user"] == user_nickname:
                ...
                if category in category_data:
                    ...
                    # 정의되지 않은 카테고리는 기타로 집계
                    category_data['기타'] += participant["amount"]

    # 유효한 데이터가 있는 카테고리만 남김
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        values=df['amount'], 
        hole=.3,
        marker=dict(colors=[fixed_colors[cat] for cat in df['category']]),  # 고정된 색상 적용
        textinfo='none'  # 그래프 안의 글자 없애기
    )])

    fig.update_layout(
        margin=dict(t=20, b=20, l=20, r=20),  # 그래프의 여백을 조정하여 그래프 크기 키우기
        plot_bgcolor='rgba(0, 0, 0, 0)',  # 배경을 투명하게 설정
        paper_bgcolor='rgba(0, 0, 0, 0)',  # 배경을 투명하게 설정
        font=dict(color='yellow'),  # 글꼴 색상을 노란색으로 설정
        showlegend=False  # 범례 숨기기
    )

    # 그래프를 이미지로 변환하여 반환
    img_bytes = fig.to_image(format="png", engine="kaleido")  # kaleido를 사용하여 투명 배경 지원
